...
func TestPrecheck_ForcedSkipsAll(t *testing.T) {
	...
	err := Precheck("bitaxe-601", binPath, "", "", true)
...
func TestPrecheck_ProjectNameMismatch(t *testing.T) {
	...
	err := Precheck("bitaxe-601", binPath, "", "", false)
	require.Error(t, err)
	assert.Contains(t, err.Error(), "does not contain board")
}

func TestPrecheck_BinarySizeOversized(t *testing.T) {
	tmpDir := t.TempDir()
	binPath := filepath.Join(tmpDir, "oversized.bin")

	// Create an oversized binary with valid app descriptor
...
func TestPrecheck_DeviceViaHTTP_Success(t *testing.T) {
	...
	err := Precheck("bitaxe-601", binPath, server.Listener.Addr().String(), "", false)
	require.NoError(t, err)
}
...
func TestPrecheck_N8T_Board(t *testing.T) {
	...
	err := Precheck("bitdsk-n8t", binPath, server.Listener.Addr().String(), "", false)
	require.NoError(t, err)
}
